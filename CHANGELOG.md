...
**検出器・`tools/patterns/target-reach.ts` の定数（`TARGET_REACH_MAX_BARS` / `MIN_TARGET_DISTANCE_HEIGHT_RATIO` /
`TARGET_REACHED_PCT_CAP`）・表示・ベースラインは 1 行も変えていない**（`git diff --stat -- tools/ src/` が空）。
`TARGET_REACH_MAX_BARS` = 60 は #210 の延べ集計（合成 fixture が大半）で「到達済み 2,576 行のうち
**60 本以内が 96.3%**」として決めた値だが、これは**到達済みケースの条件付き分布**であって、
到達**率**でも**帰無との比較**でもない。本 Phase はその 2 つを実データで測り直した。

3,764 ケース（実データ A 4 + B / C / D 各 1,224（窓 60〜365 本 × `swingDepth` 4）+ 合成 88）。
主表は**実データ 1hour の実体**（B / C / D は同じ 1 時間足履歴の重なる窓なので絶対時刻で 1 本に畳む）:

| 何を | 実測（実データ 1hour / accepted 実体） |
|---|---|
| 母集団 | 延べ 18,030 / 実体 42 / 値動き 30。`avail ≥ 60` が **38**（打ち切り 4）、到達（`firstReach ≤ 60`）が **17**、自力到達が **8** |
| **自力到達の上側** | max **40**（`rising_wedge`）、その 1 つ上は **47**（`triangle_ascending`）。**割れ目は (40, 47]** |
| **予備監査の「20 本」は保たない** | フル窓 × 既定 `swingDepth` だけなら自力到達の max は 20 だが、ローリング窓と `swingDepth` スイープで構造単位を増やすと **40** に動く。issue #288 が仮置きした 20 本はこのコーパスでは根拠にならない |
| **帰無との比較** | N=5 / 10 / 20 / 30 / 60 のどこでも**パターン起点が帰無を上回らない**（44.7% 対 53.0% @N=60、差 −8.3pt / 差の SE 8.1pt）。n = 38・帰無の試行 m = 11,590 で、**差はすべて負**（差の絶対値が差の SE を超えるのは N=10 と N=60 の 2 点だけ）。起点が重なるので二項 SE は下限の目安で、**「有意に低い」とも言えない** |
| 交絡 | 走査窓 (0, 60] に**他パターンのブレイクがある実体が 94.7%**（36 / 38）、**逆方向があるのが 44.7%**（17 / 38）。未到達 21 件のうち 47.6% は窓内に逆方向のブレイクがある |
| `bodyBackInside1_2` | 母集団 38 のうち **23 件が n/a**（`wedge_*` / `pennant` / `flag` は `neckline` を出さない）。測れた 15 件では「戻った」2 件の自力到達 **0%**、「戻らなかった」13 件の自力到達 46.2%。**n が小さすぎて申告の根拠にならない** |
| **1day（実データ A）** | `avail ≥ 60` の実体が **0 件** → **評価不能**。系列が 90 本しかなく、60 本先が揃うにはブレイク足が idx ≤ 29 に来る必要がある。「1day でも 60 で問題ない」ではなく「**1day では測れていない**」 |
| 合成 fixture | 同じく母集団 0 件（系列が短い）。**元から結論に使わない**（#219） |

**issue 本文の 2 箇所が食い違っていたので両方出した。** 「イベントの抽出」節は accepted（`invalid` / `forming` は除く）
と書いているが、「自己検算」節が固定した予備監査の 6 件のうち 3 件（`triangle_descending@22` / `@37` / `@37`）は
**`status: 'invalid'`**（下降三角形が上に抜けた形）なので、accepted だけに絞ると検算が成立しない
（実体 24 / 母集団 16 / 到達 10 になる）。主表は本文の定義どおり accepted のみ、
**`invalid` / `expired` 込み**（実体 78 / 母集団 65 / 到達 30 / 自力到達 12、自力の割れ目は (72, 83]）を別ユニットで併記した。
`detect_patterns` は `includeInvalid: true` で `invalid` を返し、そのとき `targetReachedPct` も一緒に出るので、
**走査窓の上限は `invalid` にも効いている**。どちらの母集団で判断するかは Phase 2。

**決定はしていない。** Phase 2 の選択肢は「上限を X に短縮」「60 据え置き + 他ブレイク申告」「打ち切り（何もしない）」の 3 つ。
**`structuredContent` が動かないのは 3 つ目だけ。** 1 つ目は `targetReachedPct` / `targetReached` /
`targetReachedDate` / `targetReachedPrice` の**値が変わり**、2 つ目は `otherBreakoutsBeforeReach` 相当の
**フィールドが増える**（#252 と同型の additive な差分）。**どちらもベースライン更新と #207 の履歴表への
1 行追加が要る。** 比較表は内部メモにある。

- `scripts/measure_target_reach_window_288.ts` — 計測ハーネス（新規）。`detect_patterns` を通さず検出器を直接回す
  （`globalDedup` 後の代表の入れ替わりで構造が消えるのを避けるため。#228 と同じ配慮）。
  **走査窓の現行値は本体の `TARGET_REACH_MAX_BARS` を import していて写していない。**
  `firstReach` は 60 で打ち切らず**系列末尾まで**走査する（60 本超の到達が 101 本まで実在する）。
  issue #288 の予備監査（実体 40 / 母集団 27 / 到達 18 / `firstReach > 20` の 6 件の
  `(type, firstReach)`）の自己検算が**数字を出す前**に走る
- `docs/internal/target-reach-window-288.md` — 集計表・境界・帰無の読み方・Phase 2 の選択肢
...
**理由コードは、あれば必ず併記する**（欠損時は上の表どおり基底ラベルだけ）。日本語の要約は補助で、コードが LLM と利用者の共通語彙になる
...
**検出器・`helpers.ts`・`buildTouchPivots`・ベースライン・`status` の enum は 1 行も変えていない。**
`tools/` / `src/` の差分は `tools/patterns/detect_wedges.ts` の `preparePivots` / `PivotData` に
`export` を足した **4 行だけ**で（計測が SG 平滑化列を写しではなく本体から取るため）、
判定にも出力にも影響しない。`tests/detect_patterns_data_patterns_regression.test.ts` は無差分で通る。

PR #273（#252）で `wedge_*` に出るようになった `pivots` は**上下トレンドラインの非ブレイクタッチ点
すべて**で、実データ 4 件で 31 / 37 / 68 / 107 点（`triangle_*` は 7〜9 点）。**今すぐ困る消費者は
無い**が、直すかを決めるための材料として案 A 相当の 4 つの絞り方を 12,104 ケースで並べた
（accepted wedge 延べ 24,692 / 実体 212）。

| 何を | 実測 |
|---|---|
| 点数（実体 p50 / max。実データ B / C / D） | **A0（現行）34〜43 / 92〜126**、A1 8〜9 / 15〜46、A2 4〜5 / 10〜11、A3 5〜6 / 13〜31。**合成フィクスチャでは 4 変種がほぼ同じ**（どれも 7）——点数が膨らむのは実データ固有 |
| issue の原因説明（収束区間でほぼ全バー） | **裏付けられた。** 終端の上下幅が 0 〜 0.5% の実体（33 / 212 = 15.6%）は A0 の中央値 **65**（帯が広い側は 34）、**充填率の中央値 0.88**（`A0 ÷ (走査本数 × 2)`。上下 2 本ぶんの席の 9 割が埋まっている） |
| ただし帯だけが原因ではない | 帯が狭い entry は**点数では過大に効く**（実体で entry の 15.6% が A0 合計の **26.6%**、延べで 25.0% が **37.0%**）が、それでも全体の 4 分の 1 〜 3 分の 1。帯が広い側にも A0 126 の実体があり（235 本窓）、**A0 は「帯の狭さ × 窓の長さ」の積で伸びる** |
| PR #273 の不変条件を満たさない実体 | A0 **8**（すべて「ブレイク足を含まない」）/ A1 **1**（`range` 外）/ A2 **101**（47.6% が点数 ≥ 4 を割る）/ A3 **13**（同） |
| **A0 がブレイク足を含む**（下方ブレイク） | `evaluateTouchesEx` は `isBreak` を**線ごと**に立てるので、下側を割った足の高値が上側ラインの 0.5% 以内なら**上側の非ブレイクタッチ点として残る**。実体 8 / 59（ブレイク足を持つ実体）・延べ 2,692 / 22,188。PR #273 のテストが通るのはフィクスチャが**上方**ブレイクだから |
| A1 の `price` 基準の衝突 | **回帰パスの入力点は `price` が終値**（`preparePivots` の `sgPeaks` / `origHighs`）で、`swing.ts` の `Pivot` docstring の表（wedge は**高安**）と衝突する。**形成中パスの入力点は高安**なので衝突しない。**本コーパスに回帰パス由来の accepted は 0 件**なので、衝突は実測ではなくコードを読んだ帰結 |
| A3 の「走」の定義の非恣意性（#214） | **閾値が無いのは見かけだけ。** 連続の判定に 1 本の飛びを許すと**延べ 14,548 / 24,692（58.9%）の点数が動き**、点数 ≥ 4 を新たに割る実体が 7 件できる |
| 復元の健全性 | トレンドラインの**復元失敗 0 件**。候補が割れたのは延べ 44（0.2%）——上下幅が割れた 44 件 / **A1 の入力点が割れた 12 件**（振れ幅は上下幅 最大 0.04 %ポイント・A1 の点数 最大 5 点）。分位点は動かないが、A1 の 1 件ずつの値は「候補のうちの 1 つ」 |

**Phase 2 の選択肢は A1 / A2 / A3 / 何もしない の 4 つで、決定はしていない。**
A1 〜 A3 はいずれも `pivots` の中身が変わるので、**ベースライン更新と #207 の履歴表への 1 行追加が要る**。
**「何もしない」を選んでも「A0 がブレイク足を含む」件は残る**——`docs/tools.md` の継続系の表にある
「ブレイク足は含めない」が実測と食い違っており、記述を実態に合わせるか
`buildTouchPivots` で落とすかの判断が別途要る（後者は出力が変わる）。
**案 B（`touchCount` のような集計フィールド追加）は本 Phase の対象外**（issue の指示）。

- `scripts/measure_wedge_pivot_count_274.ts` — 計測ハーネス（新規）。**トレンドラインと窓は
  写しではなく「候補を並べて A0 と突き合わせて特定する」**——本物の `evaluateTouchesEx` を当てて
  非ブレイクタッチ点の `(idx, kind)` 列が `entry.pivots` と完全一致し、かつ
  `calcApex(...).barsToApex` が `entry.daysToApex` と一致する候補だけを残す
  （選択規則は 1 行も写していないので、検出器が動けば一致しなくなって「復元できない」件数に出る）。
  issue 本文の実データ 4 件（31 / 37 / 68 / 107）の自己検算が**数字を出す前**に走る
- `docs/internal/wedge-pivot-count-274.md` — 計測結果・A1 の `price` 衝突・不変条件・Phase 2 の選択肢
...
#### 「横取り」ではなく前段への帰属変更（候補単位で追跡した）

#261 のゲートは最後尾に置いたので「既存の理由コードが 1 件も減らない」ことを要求できたが、
**単調性ゲートは前段にある**ので後段の理由コードから件数が移るのは設計どおり。

**集計値の増減だけでは「A が減って B が増えた」までしか言えない**ので、
`(ケース, type, 構成点の idx)` で配線前後の候補を対応付けて**同一候補を追跡**した
（対象は形成中 triple の候補＝`indices` が 3 点でその末尾が窓の最終足のもの）。

| 遷移（配線前 → 配線後） | 延べ |
|---|---:|
| 理由コードが変わらない | 269,037 |
| **非単調性の理由 → 単調性の理由** | **35,956** |
| 非単調性の理由 → 別の非単調性の理由 | **0** |
| 配線前だけに存在（候補ごと消えた） | **0** |
| 配線後だけに存在（ループが先へ進んで増えた） | 328 |
| 対応が一意に決まらない | 0 |

**理由コードが変わった候補は 1 件残らず単調性ゲートへ移っている。** 別の経路へ逃げた候補も、
候補ごと消えた候補も 0 件。集計値の差（単調性が得た 36,077 − 単調性以外が失った 35,749 = 328）は
「配線後だけに存在」328 件と一致し、**棄却の `continue` でループが先のペアまで回るぶん**
（§3-1 / §6-2 と同じ構造）。
...
母集団は各検出器の完成済み経路でブレイクが確定した候補（延べ 2,312 行 / 構造 143 件）。
記録フック版は 1,088 ケース全件で本物の 3 検出器と JSON 全キー一致（`baselineMismatch = 0`）。
...
| 実データ D 96 | 368 | **300**（同じ内訳で −68） | 368（差分 0） |
...
生まれているのではなく、同じ構造の右肩の取り方が動いている。** 実データ B / C / D は
追加 0 件で、入れ替わりも起きていない（純粋な削除）。

実データ B は「間に同種ピボットがある構造」が 7 件あるのに `data.patterns` の差分が 0
——いずれも `globalDedup` で既に別のパターンに負けており、検出器から消しても出力が
変わらない構造だった。

実データ C と D は**同じ値動きを違う窓で切ったもの**（346 本が重なり、D の idx + 19 が
C の idx）。落ちる構造も同一で、たとえば D の `triple_top 149-153-155-158-165` は
C の `168-172-174-177-184` と同じ 5 点——#247 が落とす `double_top` と同じ区間にある。
