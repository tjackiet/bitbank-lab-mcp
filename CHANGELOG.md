...
#### 「`1hour` では 4.8 倍ずれた double が通りえる」は成立しない（#178 項目 4 の前提の訂正）

issue の起票時の見立ては、`DOUBLE_LEVEL_MAX_PCT`（固定 3%）を `getSizeThresholdsForTf().heightPct`
（時間足別。`1hour` で 0.62%）で割って `spreadRatio` の上界を 4.8 と見積もっていた。**この上界は
到達不能。** 2 つの制約が独立ではないため:

- **幾何**: 主構成点が全構成点に含まれるので `spreadAbs <= heightAbs`、つまり
  **`spreadRatio <= 1.0`（時間足に依らない）**。実測でもサイズ検査を通過した 191 構造で
  `spreadRatio > 1` は **0 件**（max 0.951）。
- **サイズ検査の深さ条件**: 深さは谷を挟む 2 山の**平均**から測るので
  `heightAbs >= depthPct × 価格水準 + spreadAbs / 2`。`spreadRatio > 0.5` と両立するには
  `spreadAbs > depthPct / 1.525 × 価格水準` が要り、`spreadAbs` は 3% で上から抑えられている。
  したがって **`depthPct >= 4.575%` の時間足では本ゲートは幾何的に発火しえない。**
...
| `8hour` | 2.89% | 0.69 | **届く** |
| `12hour` | 3.54% | 0.60 | **届く** |
| `1day` 以上 | 5.00% | **0.467** | **届かない**（幾何的に不可能） |

**つまり `DOUBLE_LEVEL_MAX_PCT` を時間足別テーブルにする根拠も、`maxRatio` を時間足別にする
根拠も無い。** 比は無次元で上界が時間足に依らず、実際に効きうるのが短い足だけなのは
**サイズ検査の側が既に時間足別だから**であって、比の閾値を分ける理由にはならない
（`MAX_LEVEL_SPREAD_RATIO` の「時間足別テーブルを持たない理由」がそのまま当てはまる）。
上表は `tests/patterns/level-diff-double.test.ts` が `validatePatternSize` +
`getSizeThresholdsForTf` の実装から機械的に再導出して固定している。
...
到達可能 max は 0.204、合成は 0.188。上の幾何の表と一致している。
