...
| 66 | #277 | **窓の終端 `swingDepth` 本の余白でゲートが発火しないことを仕様として明文化した。docs / docstring / テストのみで検出器は 1 行も変えていない。** `detectSwingPoints` の走査範囲が `[swingDepth, length − swingDepth)` に閉じているため、**窓の終端から `swingDepth` 本以内の足はピボットになれず**、#242 の経路ゲートと再進入チェックはその区間の戻しを見ない。結果として**同じ値動きでも `limit`（窓の終端位置）によって `completed` / `invalid` が変わりうる**のを #251 と同じく仕様として固定した | **変わらない**（`tools/` / `src/` の変更は `swing.ts` の JSDoc と `swingDepth` の description 1 文だけ） |
...
**窓の終端から `swingDepth` 本以内の足はピボットになれない**。#242 の経路ゲート
...
`swingDepth` 本ちょうど内側に入った時点で `invalid` に変わる。実データ C の 365 本窓
...
- `tests/patterns/breakout-path-double.test.ts` — 終端だけ違う 2 窓（`swingDepth` 未指定）の
  `completed` / `invalid` と、実データ C の 365 本窓の `completed` を固定
- `tests/patterns/swing.test.ts` — 長さ `2 × swingDepth + 1` の系列で、終端の余白にある極値が
  ピボットにならないこと（後ろに `swingDepth` 本足すとピボットになること）の単体テスト
