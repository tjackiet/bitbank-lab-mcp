...
トリムは `accepted` を優先して残すので、**削られるのは必ず `accepted: false`＝棄却理由**。
`view=debug` の目的（なぜ検出されなかったかを理由コードで追う。#144 / #145）そのものが選択的に消える。
#152 → #167 / #172 は censored な内訳から誤帰属した実例で、censored かどうかは出力から分からなかった。
...
#176 が 20 → 32 を「cap 到達 / 超過ケース」として記録しているが、#175（86）と #176（20）は
