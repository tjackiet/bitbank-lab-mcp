...
|---|---|
| 母集団 | 延べ 18,030 / 実体 42 / 値動き 34。`avail ≥ 60` が **38**（打ち切り 4）、到達（`firstReach ≤ 60`）が **17**、自力到達が **8** |
...
**決定はしていない。** Phase 2 の選択肢は「上限を X に短縮」「60 据え置き + 他ブレイク申告」「打ち切り（何もしない）」の 3 つで、
`structuredContent` が動くのは最初の 1 つだけ（`targetReachedPct` / `targetReached` の値が変わる
= ベースライン更新と #207 の履歴表への 1 行追加が要る）。比較表は内部メモにある。
