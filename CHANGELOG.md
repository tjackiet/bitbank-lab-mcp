...
ピボット列が厳密に交互しているとき、新しい窓生成の出力は**旧実装の出力を含む**（肩 2 つの間に
肩が 1 つだけなら、それが頭で、谷は各区間に 1 つずつしかない = 旧実装と同じ 5 点）。窓が増えるのは
交互が崩れた区間だけで、**緩めた分の品質保証は窓生成ではなく後段のゲート**（サイズ検査 #139 /
構造ゲート #140 / 先行トレンド / ネックライン水平性）が担う。
...
**0 / 704 は「窓生成が死んでいる」ことを意味しない。** 合成 fixture はパターンを意図した形で
組んでおり、**ピボット列が厳密に交互している**ため、新しい窓生成は旧実装と同じ窓しか作らない
（上記の「旧実装の出力を含む」が等号で成り立つ）。配線は **ablation で確認した**——
`enumerateHsWindows` が常に空を返すようにして同じ 704 ケースを走らせた結果:
