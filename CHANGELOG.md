...
| アドバイザリ | GHSA-5jgf-p345-68v8 / GHSA-f65p-4m7j-42xc / GHSA-fph4-wmhf-6fwf / GHSA-jqff-g426-hqxp（脆弱範囲 `3.0.0 - 3.1.5`） |
| 直接依存の変更 | 無し（`ajv` の `^3.0.1` の範囲内なので上位パッケージは動かない） |
...
| 版 | 公開 | クールダウン | 脆弱範囲 `3.0.0 - 3.1.5` |
|---|---|---|---|
| 3.1.5（現行） | 33 日前 | — | **該当** |
| **3.1.6（採用）** | **10 日前** | **外（採用可）** | 非該当 |
| 3.1.7 | 0 日前 | 内（採用不可） | 非該当 |
...
**クールダウン内の 6.16.0 まで上げないと解消しない**（公開 4 日前）。CI のゲートは
`--audit-level=high` なので moderate は落とさない。7 日経過後に別途上げる。
