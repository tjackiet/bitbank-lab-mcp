...
* **（おまけ）本ドキュメント自体を AI から参照する** → GitBook が提供する MCP サーバ `https://bitbank-lab.gitbook.io/bitbank-lab-docs/~gitbook/mcp` を MCP クライアントに登録するとドキュメント横断検索が使えます（Claude Desktop の場合は `npx -y mcp-remote <URL>` 経由）
