...
> **その後（#261）**: 形成中 triple 2 経路と double 2 経路にも配線した。
> ネックライン水準は**各経路が出力に使っている値**（形成中 triple は 2 つの中間構成点の終値平均、
> 形成中 double top は谷の終値、形成中 double bottom は中間の山の終値）で、判定関数
> `validateMainPointsNecklineSide` は完成済みと共有する。理由コードだけ `forming_` 接頭辞で分けた。
> 実測は #178 の計測スクリプト §6。
...
- **H&S 系は未対応。** #211 の (i)（ブレイク検出のクランプ）の是非が決まってから別 PR。
  ...
  −5 / −9 / −10 と動く。
- **形成中経路は対象外**（本 PR 時点）。ネックラインの引き方と暫定構成点の扱いが完成済みと違うので、
  同じ判定をそのまま持ち込めない。**#261 で配線済み**——各経路が出力に使っているネックライン水準を
  そのまま渡し、暫定構成点は #169 の idiom（`{ idx, price: 終値 }`）で渡すことで解決した。
