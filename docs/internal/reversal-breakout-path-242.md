...
`scripts/measure_reversal_path_242.ts` の出力をそのまま貼ったもの（**#242 実装前の
`main`（PR #241 のマージ = `b49a08e`）の 3 検出器に対して取得**。スクリプトが
`git show` でそのリビジョンから読むので、実装後でもこの出力は再現する）。判定は `structural.ts` の
...
| # | 結論 |
|---|---|
| 1 | **double の経路ゲート（PR 1）で落ちるのは実データ C の 1 構造・実データ D の 2 構造だけ**（延べ 16 / 384 と 24 / 344）。標準コーパスと実データ B は 0 件。実データ D で落ちる `329-334-338` が**起票時のライブ実例そのもの** |
| 2 | **triple / H&S の経路ゲート（PR 2）は実データ C / D で `head_and_shoulders` −56 / `triple_top` −12**（延べ、どちらの窓でも同数）。標準コーパスでは type 別の増減 0 だが **20 行が入れ替わる**（下記） |
| 3 | **再進入チェックの横展開（PR 2）は標準コーパスの 4 構造だけ**。実データ B / C / D は 0 件。経路ゲートと**別の形**を捕まえている（両方に当たる構造は標準コーパスに 2 件、片方だけの構造も 2 件ずつある） |
| 4 | **棄却理由の帰属が変わる候補は 0 件**。ゲートは各経路の最後に置いてあり、記録位置がそれと一致している |
| 5 | **増えた type は無い。** 標準コーパスの `inverse_head_and_shoulders` は件数が同じまま構造が入れ替わる——`findStrictInverseHS` は肩の組を総当たりするので、1 つ落ちると**同じ頭・同じ谷を共有する別の右肩の窓**が `globalDedup` を勝ち抜いて出てくる（下の removed / added を突き合わせると `…-53-56` → `…-53-80` のように右肩だけが動いている）。**新しい形が生まれているのではなく、同じ構造の右肩の取り方が 1 つ後ろにずれている**。実データ B / C / D では追加 0 件 |

## 起票時のライブ実例（実データ D）

issue #242 の再現手順（`limit=72` を 2026-09-05 に実行）の窓は実データ D の idx 288〜359。
本計測はその窓ではなく **365 本まるごと**を 3 時間足 × `swingDepth` 4 種で掃いているが、
同じ構造は 365 本の窓でも `double_top 329-334-338`（山2 338 / ブレイク 344 / 間の H ピボット
**343** = `2026-09-04T11:00Z` / 終値 12,711,037 / 高安 12,731,234）として現れ、経路ゲートで落ちる。

`limit=72` の窓そのものでの再現（`double_top 41-46-50` が `peak_after_last_pivot` で落ち、
`triangle_ascending`（`status: invalid`）だけが残る）は
`tests/patterns/breakout-path-double.test.ts` が固定している。
...
**実装後でもそのまま再現する。** スクリプトは検出器のソースを作業ツリーからではなく
`git show <rev>:tools/patterns/<file>` で**ベースラインのリビジョンから**読む
（既定は `b49a08e` = PR #241 のマージ＝ #242 実装前の `main`）。

- `--baseline-rev <rev>` — ベースラインを明示指定する
- `--baseline-rev worktree` — 作業ツリーから読む。**実装後は差分が 0 になるのが正しい**
  （注入したゲートが冪等な no-op になる。冪等性の検算にはなるが Phase 1 の差分は出ない）

作業ツリーを既定にしていない理由は 1 つで、`gate='off'` が無効化するのは**注入したフックだけ**
だから。複製元が本番のゲート（`checkBreakoutPath` / `applyPostBreakoutGates`）を持っていると
**ベースライン自体が既にゲート済み**になり、差分が常に 0 なのに数字だけは出る。
スクリプトはこの状態を検出して**落ちる**（`worktree` を明示したときを除く）。
...
| ケース数 | 1088 |
| ゲート位置に到達した候補（延べ） | 2312 |
| **記録フック版が素の複製と食い違ったケース** | **0** |
| accepted 合計（ゲート無し） | 1544 |
...
| type / 経路 | 延べ行 | 構造 | 間に同種ピボット有り（構造） | ゾーン再進入（構造） |
|---|---:|---:|---:|---:|
| double_top / strict | 56 | 2 | 2 | 0 |
| head_and_shoulders / strict | 408 | 20 | 20 | 0 |
| inverse_head_and_shoulders / strict | 192 | 18 | 7 | 0 |
| triple_bottom / strict | 24 | 2 | 0 | 0 |
| triple_top / strict | 24 | 1 | 1 | 0 |
| **合計** | **704** | **43** | **30** | **0** |
...
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 4hour / swingDepth=auto
  - 構成点 idx: `[65, 72, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 4hour / swingDepth=auto
  - 構成点 idx: `[57, 72, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 4hour / swingDepth=auto
  - 構成点 idx: `[19, 49, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 4hour / swingDepth=2
  - 構成点 idx: `[11, 49, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 310（2026-09-03T02:00:00.000Z / 終値 12,302,418 / 高安 12,319,998） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 4hour / swingDepth=2
  - 構成点 idx: `[4, 49, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 310（2026-09-03T02:00:00.000Z / 終値 12,302,418 / 高安 12,319,998） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **double_top / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[155, 158, 165]` / 最終構成点 165 → ブレイク 179
  - 間の同種ピボット: idx 175（2026-08-28T11:00:00.000Z / 終値 12,725,937 / 高安 12,762,331）
  - ゾーン再進入: なし
- **double_top / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[329, 334, 338]` / 最終構成点 338 → ブレイク 344
  - 間の同種ピボット: idx 343（2026-09-04T11:00:00.000Z / 終値 12,711,037 / 高安 12,731,234）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[212, 220, 228, 235, 274]` / 最終構成点 274 → ブレイク 294
  - 間の同種ピボット: idx 283（2026-09-01T23:00:00.000Z / 終値 12,416,308 / 高安 12,437,337） , idx 289（2026-09-02T05:00:00.000Z / 終値 12,428,578 / 高安 12,541,002）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[208, 220, 228, 235, 274]` / 最終構成点 274 → ブレイク 294
  - 間の同種ピボット: idx 283（2026-09-01T23:00:00.000Z / 終値 12,416,308 / 高安 12,437,337） , idx 289（2026-09-02T05:00:00.000Z / 終値 12,428,578 / 高安 12,541,002）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[212, 220, 228, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[198, 200, 228, 235, 274]` / 最終構成点 274 → ブレイク 294
  - 間の同種ピボット: idx 283（2026-09-01T23:00:00.000Z / 終値 12,416,308 / 高安 12,437,337） , idx 289（2026-09-02T05:00:00.000Z / 終値 12,428,578 / 高安 12,541,002）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[208, 220, 228, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[188, 194, 228, 235, 274]` / 最終構成点 274 → ブレイク 294
  - 間の同種ピボット: idx 283（2026-09-01T23:00:00.000Z / 終値 12,416,308 / 高安 12,437,337） , idx 289（2026-09-02T05:00:00.000Z / 終値 12,428,578 / 高安 12,541,002）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[198, 200, 228, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[139, 141, 165, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[83, 85, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[65, 72, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
...
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  ...
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **triple_top / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=auto
  - 構成点 idx: `[149, 153, 155, 158, 165]` / 最終構成点 165 → ブレイク 180
  - 間の同種ピボット: idx 175（2026-08-28T11:00:00.000Z / 終値 12,725,937 / 高安 12,762,331） , idx 179（2026-08-28T15:00:00.000Z / 終値 12,552,117 / 高安 12,771,695）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[30, 39, 49, 65, 72]` / 最終構成点 72 → ブレイク 80
  - 間の同種ピボット: idx 76（2026-08-24T08:00:00.000Z / 終値 12,272,447 / 高安 12,252,345）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[25, 39, 49, 65, 72]` / 最終構成点 72 → ブレイク 80
  - 間の同種ピボット: idx 76（2026-08-24T08:00:00.000Z / 終値 12,272,447 / 高安 12,252,345）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[42, 45, 49, 83, 88]` / 最終構成点 88 → ブレイク 94
  - 間の同種ピボット: idx 91（2026-08-24T23:00:00.000Z / 終値 12,557,431 / 高安 12,519,594）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[30, 39, 49, 83, 85]` / 最終構成点 85 → ブレイク 94
  - 間の同種ピボット: idx 88（2026-08-24T20:00:00.000Z / 終値 12,557,106 / 高安 12,505,833） , idx 91（2026-08-24T23:00:00.000Z / 終値 12,557,431 / 高安 12,519,594）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[25, 39, 49, 83, 85]` / 最終構成点 85 → ブレイク 94
  - 間の同種ピボット: idx 88（2026-08-24T20:00:00.000Z / 終値 12,557,106 / 高安 12,505,833） , idx 91（2026-08-24T23:00:00.000Z / 終値 12,557,431 / 高安 12,519,594）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[30, 39, 49, 83, 88]` / 最終構成点 88 → ブレイク 94
  - 間の同種ピボット: idx 91（2026-08-24T23:00:00.000Z / 終値 12,557,431 / 高安 12,519,594）
  - ゾーン再進入: なし
- **inverse_head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[25, 39, 49, 83, 88]` / 最終構成点 88 → ブレイク 94
  - 間の同種ピボット: idx 91（2026-08-24T23:00:00.000Z / 終値 12,557,431 / 高安 12,519,594）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=2
  - 構成点 idx: `[134, 136, 165, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 310（2026-09-03T02:00:00.000Z / 終値 12,302,418 / 高安 12,319,998） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
- **head_and_shoulders / strict** — `btc_jpy_1hour_2026_09_05` / 1hour / swingDepth=6
  - 構成点 idx: `[19, 49, 94, 285, 289]` / 最終構成点 289 → ブレイク 317
  - 間の同種ピボット: idx 300（2026-09-02T16:00:00.000Z / 終値 12,268,848 / 高安 12,326,669） , idx 313（2026-09-03T05:00:00.000Z / 終値 12,260,000 / 高安 12,320,000）
  - ゾーン再進入: なし
...
**経路ゲート / double のみ（PR 1）** — accepted 延べ 344（差分 removed 24 / added 0）

| type | before | after | 増減 |
|---|---:|---:|---:|
| double_top | 28 | 4 | -24 |

- removed（構造）: `btc_jpy_1hour_2026_09_05/1hour double_top 155-158-165` , `btc_jpy_1hour_2026_09_05/1hour double_top 329-334-338`
- added（構造）: なし

**経路ゲート / triple + H&S（PR 2）** — accepted 延べ 300（差分 removed 68 / added 0）
...
- removed（構造）: `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 11-49-94-285-289` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 188-194-228-235-274` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 198-200-228-285-289` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 65-72-94-285-289` , `btc_jpy_1hour_2026_09_05/1hour triple_top 149-153-155-158-165` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 11-49-94-285-289` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 19-49-94-285-289` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 65-72-94-285-289`
- added（構造）: なし

**再進入チェック横展開 / triple + H&S（PR 2）** — accepted 延べ 368（差分 removed 0 / added 0）

| type | before | after | 増減 |
|---|---:|---:|---:|
| （type 別の増減なし） | | | 0 |

**全部（経路 + 再進入横展開）** — accepted 延べ 276（差分 removed 92 / added 0）
...
- removed（構造）: `btc_jpy_1hour_2026_09_05/1hour double_top 155-158-165` , `btc_jpy_1hour_2026_09_05/1hour double_top 329-334-338` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 11-49-94-285-289` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 188-194-228-235-274` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 198-200-228-285-289` , `btc_jpy_1hour_2026_09_05/1hour head_and_shoulders 65-72-94-285-289` , `btc_jpy_1hour_2026_09_05/1hour triple_top 149-153-155-158-165` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 11-49-94-285-289` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 19-49-94-285-289` , `btc_jpy_1hour_2026_09_05/4hour head_and_shoulders 65-72-94-285-289`
- added（構造）: なし
