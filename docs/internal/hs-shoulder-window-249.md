...
本メモの Phase 1 を受けて、**右肩の取り方・探索窓（`HS_BREAKOUT_MAX_BARS = 30`）・全長上限のいずれもコードは変えない**（issue #249 のコメント）。

- 目視で「0 件が正しい」と確定した。形 A / B は頭と両肩の差が 1.3% 程度で、間に両肩より高い山が 3 つある 4 山のレンジ。#242 のゲートは実在する H&S を巻き添えにしていない。
- 案 A（右肩の優先順位）は候補集合に無いものを選べないので実装できない。右肩を取り直すと `extremeBetween` で谷2 も動き、形 A は形 B に畳み込まれ、形 B は構造ゲートかブレイク不成立で落ちる。
- 案 B（探索窓の時間足別化）は落ちる構造が同じで、理由コードが `peak_after_last_pivot` から `near_completion` に移るぶん `view=debug` の診断が悪くなる。
- **案 C（完成済み経路の全長上限）は次点として残す。** 入れると実データ B の `inverse_head_and_shoulders 3-9-42-147-154`（左肩→右肩 151 本 > forming の上限 146）が消える。判断材料がこの 1 件だけなので、#244（時間足別テーブル）で一緒に再検討する。

帰結は `docs/tools.md`（「完成済み H&S のブレイク探索窓は 30 本固定」の節）と `tools/patterns/detect_hs.ts` の `HS_BREAKOUT_MAX_BARS` の docstring に書いた。#251（経路ゲートの深さ依存・案 3）と合わせて 1 本の文書 PR で閉じている。
