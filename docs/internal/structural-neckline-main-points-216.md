...
`price` 基準・ネックライン `scalar`（ゲート発火 942 回。[4-2-1 章](#4-2-1-発火回数-942-と生の行数-773-の関係)で内訳を出す）:
...
#### 4-2-1. 発火回数 942 と生の行数 −773 の関係

**「ゲート発火」は候補単位（`m216Reject` が reject を返した回数）で、
[計測 1](#1-2-940-ケース全体) の 773 は「ベースラインで emit された行のうち誤側だったもの」**という
別の単位。突き合わせると **942 = 773 + 96 + 73** に分解できる（`price` × `scalar`）:

| 内訳 | 件数 | 中身 |
|---|---:|---|
| ベースラインの emit 行に対応する発火 | **773** | **計測 1 の誤側行 773 と完全一致**（発火が無い誤側行は 0 件、行が無いのに発火した重複も 0 件） |
| 同一候補への重複発火 | **96** | strict が落ちて relaxed 段が走り、**同じ `(a, b, c)` にもう一度発火する**分。発火した distinct 候補は 816 |
| ベースラインでは行にならない候補への発火 | **73** | 内訳は下記 |

「行にならない候補」73 の内訳（全件 triple。double / H&S は 0 件）:

| 経路 | 件数 | 理由 |
|---|---:|---|
| strict | **43** | **全件が `near_completion` の triple 候補。** `detectTriples`（`detect_triples.ts` の `includeForming` フィルタ）が `includeForming=false` のとき `near_completion` を返さないため、ゲート位置には到達するが `patterns` に載らない |
| relaxed | **30** | ベースラインでは走らない relaxed 段（strict が 0 件になって初めて走る）で到達した候補 |

**したがって「942 件が落ちる」と読んではいけない。** 検出器層で実際に減るのは
生の行数の Δ（`double_bottom` −4 / `triple_top` −88 / `triple_bottom` −20 /
`head_and_shoulders` −46 / `inverse_head_and_shoulders` −615 = **−773**）で、
発火回数はゲートを何回引いたかの計数でしかない。

6 変種すべてで同じ突き合わせをすると、**「行にならない候補への発火 73」と
「重複発火 96」は `price` の 3 モードで完全に同一**（どちらも double / triple 由来で、
その 2 type のネックラインは水平＝モード間で判定が変わらないため）。
`extremePrice` 基準では double / triple が 1 件も落ちず relaxed 段も追加で走らないので、
**発火回数と生の行数 Δ が一致する**:

| 基準 | 発火回数 | ＝ 生の行数 Δ | ＋ 重複発火 | ＋ 行にならない候補 |
|---|---:|---:|---:|---:|
| `price` × `scalar` | 942 | 773 | 96 | 73 |
| `price` × `line` | 994 | 825 | 96 | 73 |
| `price` × `clamped` | 501 | 332 | 96 | 73 |
| `extreme` × `scalar` | 458 | 458 | 0 | 0 |
| `extreme` × `line` | 116 | 116 | 0 | 0 |
| `extreme` × `clamped` | 4 | 4 | 0 | 0 |
