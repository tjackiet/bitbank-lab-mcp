...
**本文で数える対象は double の第2構成点 / triple の第2・第3構成点 / H&S の頭・右肩**で、
H&S の第2中間点 (p3) は検査されていない点ではあるが**ネックラインの定義点そのもの**なので
[1-5 章](#1-5-参考-p3第2中間点)に参考として分離した（issue の「p3 は `mid` と同じ役割なので
参考として別集計」に従う）。

> **H&S の数値は #211 前の基準・母集団で取ったもので、Phase 2（H&S 分）には使えない。**
> `necklineAt` の外挿クランプ（#211 / PR #239）が入った後の基準で測り直した結果は
> [`hs-neckline-side-216.md`](./hs-neckline-side-216.md)。本ログの `clamped` 列は
> 「#211 が入ったらこうなる」という**参考値**で、母集団（#211 前の検出結果）が違うため
> 再計測の値とは一致しない。**`double` / `triple` の数値は水平ネックラインなので基準に
> 依存せず、そのまま有効**（Phase 2 の判断根拠として #223 が使っている）。
...
1. **誤った側に来るのは実質「構成点列の最後の主構成点」だけで、頭はほぼ 0 件。**
   役割別の内訳（生の行数）では `triple_*` の第2構成点 0 件 / 第3構成点 108 件に対し、
   H&S 系の頭は 4 基準中 3 基準で 0 件、残る 1 基準（`price` × 線）でも
   `inverse_head_and_shoulders` の 1,990 行中 **6 行**だけ。右肩は 116〜707 件（基準による）。
   **「頭がネックラインを割っている」形は、その 6 行（`1min` ラベルの退化した 3 構造）を除けば
   本コーパスに存在しない**（[1-3 章](#1-3-役割別どの点が誤った側に来るのか)）。
...
6. **ゼロ許容 hard gate の試算は `data.patterns` 全体で 0 〜 −70 件**（基準による。
   `extremePrice` × 外挿クランプ付きの線は `data.patterns` に対して no-op）。
...
| #205 | コーパス層 896 の生の行数 / `(系列,type,idx)` | 2,608 / 119 | **2,608 / 119** |
...
double の第2構成点、H&S の右肩。中間の第2構成点（triple）は全 4 基準で 0 件、
頭（H&S）は **`price`×`line` の逆 H&S の 6 行を除いて 0 件**。
その 6 行は[付録](#付録-誤った側にある構造の全件明細)の No.91 / 93 / 95——頭 (idx 331) が
ネックライン定義点 p1 (330) と p3 (339〜355) に挟まれた区間の端にある退化した形で、
`1min` ラベルのみに出る（3 構造 × 2 `swingDepth` = 6 行）。
...
`price` 基準・ネックライン `scalar`（ゲート発火 942 回。[4-2-1 章](#4-2-1-発火回数候補数生の行数の関係)で内訳を出す）:
...
`extremePrice` 基準（発火 458 / 116 / 4 回）。**double / triple はどのモードでも Δ 0**:

| ネックライン | 生の行数 Δ（H&S / 逆H&S） | 構造単位 Δ | dedup後 構造 Δ | `data.patterns` Δ（H&S / 逆H&S） | `data.patterns` 全体 Δ |
|---|---:|---:|---:|---:|---:|
| `scalar` | −3 / −455 | −1 / −121 | 0 / −3 | 0 / −5 | **−5** |
| `line` | −2 / −114 | −1 / −48 | −1 / **+3** | 0 / −1 | **−1** |
| `clamped` | 0 / −4 | 0 / −4 | 0 / 0 | 0 / 0 | **0** |

**`extremePrice` 基準では double / triple が 1 件も落ちない**——[1-3 章](#1-3-役割別どの点が誤った側に来るのか)で見たとおり、
ヒゲを含めれば主構成点はネックラインの正しい側に収まっている。
`extremePrice` × `clamped` は検出器層で 4 行落ちるだけで、**`data.patterns` は 1 件も変わらない**。

#### 4-2-1. 発火回数・候補数・生の行数の関係

**3 つの単位を混同しないこと。** `price` × `scalar` の場合:

| 単位 | 値 | 定義 |
|---|---:|---|
| **生の行数 Δ** | **−773** | ベースラインで emit された行のうちゲートが落とすもの。[計測 1](#1-2-940-ケース全体) の誤側行と同じ単位 |
| **distinct 候補** | **816** | ゲートが reject を返した候補を **`(ケース, type, 構成点の idx)`** で畳んだ数 |
| **発火回数** | **942** | `m216Reject` が reject を返した回数（同じ候補が複数回ゲートを引くことがある） |

**候補単位: 816 = 773 + 43**

| 内訳 | 候補 | 中身 |
|---|---:|---|
| ベースラインで emit された候補 | **773** | **誤側の行 773 と 1 対 1**（発火が無い誤側行 0 件、行が無いのに発火した候補との重複 0 件） |
| ベースラインでは emit されない候補 | **43** | 全件が `near_completion` の triple。`detectTriples`（`detect_triples.ts` の `includeForming` フィルタ）が `includeForming=false` のとき `near_completion` を返さないため、ゲート位置には到達するが `patterns` に載らない。`includeForming=false` のケースでのみ発生することを実測で確認（43/43） |

**発火回数: 942 = 816 + 126（同一候補への重複発火）**

重複の中身は 1 種類しかない。**`detectTriples` の relaxed は 2 段（factor 1.25 / 2.0）**で、
strict がゲートで 0 件になると両段とも走る。同じ `(a, b, c)` が
**strict 1 回 + relaxed 2 回 = 3 回**ゲートを引く:

| 発火回数 | 候補 | 経路の並び |
|---:|---:|---|
| 1 回 | 753 | **全件が strict のみ** |
| 3 回 | 63 | **全件が `strict + relaxed + relaxed`** |

**relaxed だけで発火する候補は 0 件**——relaxed の走査対象は strict と同じピボット列なので、
relaxed が到達する候補は必ず strict でも到達している。

`816 = 753 + 63`、`942 = 753 + 63 × 3`。63 候補の内訳は
**emit された 773 候補のうち 48（重複 96）/ emit されない 43 候補のうち 15（重複 30）**。

**したがって「942 件が落ちる」と読んではいけない。** 検出器層で実際に減るのは
生の行数の Δ（`double_bottom` −4 / `triple_top` −88 / `triple_bottom` −20 /
`head_and_shoulders` −46 / `inverse_head_and_shoulders` −615 = **−773**）だけで、
816 と 942 はゲートを何回引いたかの計数でしかない。

6 変種すべてで同じ突き合わせをすると、**「emit されない候補 43」と「重複発火 126」は
`price` の 3 モードで完全に同一**（どちらも triple 由来で、triple のネックラインは水平＝
モード間で判定が変わらないため）。`extremePrice` 基準では double / triple が 1 件も落ちず
relaxed 段も追加で走らないので、**3 つの単位がすべて一致する**:

| 基準 | 発火回数 | distinct 候補 | emit された候補（＝ 生の行数 Δ） | emit されない候補 | 重複発火 |
|---|---:|---:|---:|---:|---:|
| `price` × `scalar` | 942 | 816 | 773 | 43 | 126 |
| `price` × `line` | 994 | 868 | 825 | 43 | 126 |
| `price` × `clamped` | 501 | 375 | 332 | 43 | 126 |
| `extreme` × `scalar` | 458 | 458 | 458 | 0 | 0 |
| `extreme` × `line` | 116 | 116 | 116 | 0 | 0 |
| `extreme` × `clamped` | 4 | 4 | 4 | 0 | 0 |

**どの変種でも「発火が無かった誤側行」は 0 件**——ゲートは計測 1 が数えた行を
取りこぼしなく捕捉している。
