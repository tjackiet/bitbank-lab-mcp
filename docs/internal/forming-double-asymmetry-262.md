...
`scripts/measure_forming_double_asymmetry_262.ts` の出力（§11 に verbatim）に、読み方（§1）・
結論（§2）・問題設定（§3）・目視判定（§8）・契約の棚卸し（§9）・案の比較（§10）を足したメモ。
...
- **経路の同定を理由コードの名前でやっていない。** 形成中経路の棄却理由には完成済み経路と
  ...
  | ビルド | 中身 | 用途 |
  |---|---|---|
  | `base` | 本体は 1 文字も変えない（末尾に `export { … }` を 1 行足すだけ） | §4 / §5 の分布。作業ツリーと 12,104 ケース全件で一致することを §11-0 が検算 |
...
> 母集団ごとの内訳は §11 の §1-1〜§1-7。**プールしない**のが原則なので、この合計表は
...
ファネルは母集団ごとに §11 の §2-1〜§2-7。ループ経路なので候補 1 件 = ループ 1 周であり、
...
> 案 A は「非対称の解消」ではなく「形成中 double の廃止」と同義になる（§10）。
...
ローリング窓 B `expired` 5・`forming` 2・`invalid` 3 / C・D も同規模（§11 の §4-1〜§4-7）。
...
| 形 | 系列 / 構成点 idx（山1 - 谷 - 山2） | 終端 idx | §11 §6 の行 |
|---|---|---:|---|
...
#178 項目 1 が形成中 triple で見つけた「形成中として出るのに完成した瞬間に高さ相対で落ちる」
...
コードを読んで作った静的な表。**計測ではないので §11 の出力には出ない。**
...
- `expired` の description は「第2構成点の確定から突破確認窓を過ぎた」と書いてあり、
  **第2構成点が確定していることを前提にしている。** 案 A を採ると形成中の第2構成点が
  暫定値になるので、この文が double について成立しなくなる（§10 の案 A の「居場所を失う」の実体）。
...
- 展開先: `/tmp/forming-double-262-cutei2`（作業ツリーは 1 バイトも変更していない）
...
| 3 | `1hour|double_top|2026-08-27T15:00:00.000Z-2026-08-27T18:00:00.000Z-2026-08-28T01:00:00.000Z` | `expired` | btc_jpy_1hour_2026_09 btc_jpy_1hour_2026_09_05 / sd=2,3,auto / end=364 | 12,799,028 | 12,766,911 | 12,839,153 | 12,414,587 | 0.313% | — |
| 4 | `1hour|double_top|2026-09-03T21:00:00.000Z-2026-09-04T02:00:00.000Z-2026-09-04T06:00:00.000Z` | `expired` | btc_jpy_1hour_2026_09_05 / sd=2,3,6,auto / end=364 | 12,718,980 | 12,617,594 | 12,639,245 | 12,465,523 | 0.627% | — |
...
| 13 | `1hour|double_top|2026-08-31T19:00:00.000Z-2026-09-01T02:00:00.000Z-2026-09-01T05:00:00.000Z` | `expired` | btc_jpy_1hour_2026_09 btc_jpy_1hour_2026_09_05 / sd=2,3,auto / end=308 | 12,598,466 | 12,536,428 | 12,659,390 | 12,428,578 | 0.481% | — |
