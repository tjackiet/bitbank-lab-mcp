...
const PeriodPerformanceSchema = z
	.object({
		...
		adjusted_change_jpy: z
			...
			.nullable()
			.describe(
				'調整後増減額 = change_jpy - net_flow_jpy（入出金元本の影響を除いた成績。出金手数料コストは含む）。null=純入出金が未計測のため算出不能。**change_jpy_overstated=true のときは change_jpy と同じ額だけ過大**（純入出金は期初評価額と無関係なので過大分がそのまま残る）',
			),
		...
		change_pct_unavailable_reason: PortfolioChangePctUnavailableReasonEnum.optional().describe(
			'change_pct / adjusted_change_pct を出せなかった理由。start_value_zero=期初評価額が 0、start_value_negligible=期初評価額が現在評価額の 1% 未満（相対基準。絶対額固定は口座規模に依存するため採らない）、start_boundary_unpriced=期初の始値を解決できなかった暗号資産があり期初評価額が過小（unpriced_start_assets を参照。start_value_negligible とは原因が異なる）。設定されている場合 change_pct は undefined で、adjusted_change_pct は undefined（純入出金が未計測なら null が優先）。増減額 change_jpy / adjusted_change_jpy はどの理由でも出るが、**成績として読めるのは start_value_zero / start_value_negligible のときだけ**（期初評価額が本当に小さいだけで分子は正しい）。start_boundary_unpriced では分子も同じ欠損で過大になるため読めない（change_jpy_overstated=true が付く）。率を出せている場合は undefined',
		),
		change_jpy_overstated: z
			.boolean()
			.optional()
			.describe(
				'change_jpy / adjusted_change_jpy が過大であることの申告。true になるのは change_pct_unavailable_reason=start_boundary_unpriced のときだけで、期初の始値を解決できず start_value_jpy から脱落した評価額（資産名は unpriced_start_assets）がそのまま増減額の過大分になっている状態を指す。**このとき増減額を運用成績として読まないこと**——率を抑止した欠損がそのまま分子の欠損であり、率より壊れた数字になる。この期間の値動きはこの結果からは算出できない（*_equity_series も同じ 1day candle の始値に依存し、引けなかった日付は現在価格で代替されるため代替にならない）。値を null にせず出したうえでフラグを立てるのは、start_value_jpy を「過小でも出して申告する」とした方針と揃えるため（change_jpy = current_value_jpy - start_value_jpy で消費者が再計算できるので、null 化は契約を壊すだけで隠蔽にならない）。過大でない場合は undefined（false は返さない）',
			),
	})
	.optional();
