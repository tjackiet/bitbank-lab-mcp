...
2. web_search — ニュース取得用。呼び出しは「保有銘柄数 + 1」回まで。鮮度は業界14日・銘柄7日以内。一次ソース優先（Reuters / CoinDesk / Bloomberg / 公式ブログ等）、価格予想・煽り記事・SEOアグリゲータは除外。
...
5. 資産構成比率（横棒、銘柄色分け）
6. 📰 マーケットニュース（後述）
7. テクニカル（technical がある場合のみ）
8. 入出金サマリー（available 時: 年次・月次2カード）
9. 注意書き（指標定義・税務免責: 所得税計算に使えない）

【マーケットニュース（セクション6）】
- 6-1. 🌐 業界ニュース 2〜3件（規制・ETF・大手機関の動き等。価格変動そのものは対象外）
- 6-2. 📰 銘柄別ニュース 0〜2件/銘柄（アップグレード・上場・規制・重大事故等）
  - 質の高いニュースが無い銘柄は行ごと省略（"なし"と書かない）
  - JPY のみ保有なら 6-2 は省略
- 各項目: 見出し（自然な日本語に要約）／公開日／出典名／1〜2行の要約／URL
- 引用は15語未満。長文コピペ禁止。推測・価格予想は載せない
